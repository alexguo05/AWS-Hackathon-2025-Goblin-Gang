...
try:
    import cv2
    print("[webots] cv2:", cv2.__version__)
except Exception as e:
    print("[webots] cv2 import failed:", e)
...
class CirclePatrolDrone(Robot):
    ...
    def __init__(self):
        ...
        self.target_precision = 0.7  # smaller -> less sloppy
        # ...
        self.camera_pitch_motor = self.getDevice("camera pitch")
        self.camera_pitch_motor.setPosition(0.5)
        try:
            self.camera_yaw_motor = self.getDevice("camera yaw")
            self.camera_yaw_motor.setPosition(math.pi/2)  # exactly 90° left
        ...
        # Mission parameters
        self.target_center = [236, -524]  # Center of circle
    ...
    def run(self):
        # --- tunables for smooth orbit ---
        ORBIT_SPEED_PITCH = 0.22     # forward tilt magnitude (m/s-ish). controller expects NEG for forward
        K_YAW = 0.9                  # how hard to turn toward desired heading
        MAX_YAW_DIST = 0.5           # cap yaw authority (<= self.MAX_YAW_DISTURBANCE)
        K_RAD = 0.8                  # radial correction strength (0.4–1.2 is typical)
        RAD_DB = 0.5                 # deadband on radius (m); don't fuss if we're within this
        PHOTO_ANGLE_TOL = math.radians(12)  # take photo when left side is within ±12° of the center line
        PHOTO_RAD_TOL = 0.8                 # and radius is close
        PHOTO_PERIOD = 2.0                  # seconds between shots

        t_last_photo = 0.0
        print("🚁 Starting CIRCLE orbit (no waypoints).")
        print(f"🎯 Center={self.target_center}, R={self.circle_radius} m, Alt={self.target_altitude} m")
        ...
        while self.step(self.time_step) != -1:
            ...
            self.set_position([x, y, z, roll, pitch, yaw])

            # Altitude hold first
            at_alt = (z > self.target_altitude - 1.0)

            yaw_disturbance = 0.0
            pitch_disturbance = 0.0
            roll_disturbance = 0.0  # unused but left for completeness
            ...
            if at_alt:
                ...
                if r < 1e-6:
                    r = 1e-6
                bearing = math.atan2(dy, dx)   # direction center->drone

                # Unit vectors: radial (outward) and CCW tangent
                urx, ury = math.cos(bearing), math.sin(bearing)         # radial out
                tx,  ty  = -math.sin(bearing), math.cos(bearing)        # CCW tangent

                # Desired velocity direction = tangent + radial correction
                # Positive (r - R) -> we're outside -> steer slightly inward
                rad_err = r - self.circle_radius
                if abs(rad_err) < RAD_DB:
                    rad_term = 0.0
                else:
                    rad_term = -K_RAD * rad_err  # negative if outside (steer inward), positive if inside

                # Compose a desired heading vector
                vx = tx + rad_term * urx
                vy = ty + rad_term * ury
                ...
                yaw_disturbance = clamp(K_YAW * yaw_err, -MAX_YAW_DIST, MAX_YAW_DIST)

                # Move forward; reduce thrust if we're badly misaligned to avoid side-slips
                align = max(0.0, math.cos(abs(yaw_err)))  # 1 when aligned, 0 when 90° off
                pitch_disturbance = -ORBIT_SPEED_PITCH * align  # negative => forward in this controller

                # Photo trigger: left side faces center when (yaw + 90°) ≈ bearing
                left_facing_err = wrap_pi((yaw + math.pi/2) - bearing)
                now = self.getTime()
                if abs(left_facing_err) < PHOTO_ANGLE_TOL and abs(rad_err) < PHOTO_RAD_TOL:
                    if now - t_last_photo >= PHOTO_PERIOD:
                        self.save_camera_image()
                        t_last_photo = now

            # Motor mixing (same as example)
            ...
            clamped_diff_alt = clamp(self.target_altitude - z + self.K_VERTICAL_OFFSET, -1, 1)
